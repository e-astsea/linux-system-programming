    ...
    int sha1(char *filename, char *md5str);
    ...
    void command_fmd5( char* argv[]){          // argc argv로 명령어   
       ...
       if(argvstart[0]=='~'){                           // ~ 입력했을떄 임시파일 삭제
                strcat(deltxt,argvstartpoint);
                strcat(deltxt,"/");
                strcat(deltxt,makedeletefile);
        }
        else{
                strcat(deltxt,argv4point);              // 일반 상대,절대경로입력시 임시파일 삭제
                strcat(deltxt,"/");
                strcat(deltxt,makedeletefile);
        }
...
void option(List* linklist){
        ...
        for (int i = 0; i < count; i++) {
            ...
            if(S_ISDIR(statbuf.st_mode)){   //디렉토리인것만 큐에넣기
                ...
                if(strcmp(file_extension,"*")==0){                                          //파일확장자가 * 이면 모든파일 탐색
                    sha1(namelist[i]->d_name,filenamehash);                                  //맞는파일들의 sha1를 탐색함
    ...
    int sha1(char *filename, char *str)                                  //sha1 함수
    {
        SHA_CTX ctx;
        struct stat statbuf;
        char *hash;
        ...
        SHA1_Init(&ctx);                                                 // sha1 사용법
        SHA1_Update(&ctx, hash, statbuf.st_size);                        // sha1 사용법
        SHA1_Final(sha1, &ctx);                                           // sha1 사용법
